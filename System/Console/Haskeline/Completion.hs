...
completeWordWithPrev esc ws f (line, _) = do
    let (word, rest)   = case esc of
        ...
        (word', rest') = if word /= ""
                            then (word, rest)
                            -- if word is an empty string it is likely we have
                            -- to deal with operator, so let's try to take an
                            -- operator symbols sequence
                            else handleOperatorCase rest
  ...
  where
    ...
    handleOperatorCase s =
      let (w1, r1) = span (`elem` operatorSymbols) s
          (w2, r2) = break (`elem` ws) r1
      in (w1 ++ w2, r2)
    operatorSymbols = "!#$%&*+./<=>?@\\^|-~:"
